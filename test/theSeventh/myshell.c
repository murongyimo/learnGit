/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2016年04月26日 星期二 21时02分04秒
 ************************************************************************/
...
void do_cmd(int argcount,char arglist[100][256])
{
    ...
    int background = 0;     //标识命令中是否有后台运行符&
    int status;
    int i;
    int fd;
    char * arg[argcount+1];
    ...
    * 或者命令格式不正确。*/
    ...
    /*命令中只有一个管道符号
    * 将管道符号和后面的部分存入argnext中，管道后面的部分是一个可执行的shell命令*/
    if(how == have_pipe){
        for(i = 0;arg[i] != NULL;i++ ){
            if(strncmp(arg[i],"|",1) == 0){
                arg[i] = NULL;
    ...
    switch(how){
        case 0:
            /*pid为0,则为子进程，在子进程中执行输入的命令（命令中不含>,<,|）*/
            if(pid == 0)
             {
        ...
        case 3:
            if(pid == 0){
                ...
                if((pid2 = fork()<0)){
                    ...
                    fd2 = open("/tmp/youdonotknowfile",O_RDWR|O_CREAT|O_TRUNC,0644);
                    dup2(fd,1);
                    execvp(arg[0],arg);
                    exit(0);  
                }
...
int find_command(char *command)
{
    ...
    /*使当前目录下的程序可以运行，如命令./fork可以被正确的执行*/
    if(strncmp(command,"./",2) == 0)
        command = command + 2;
    /*分别在当前目录，/bin目录，/usr/bin目录查找要执行的程序*/
    int i = 0;
    while(path[i] != NULL){
